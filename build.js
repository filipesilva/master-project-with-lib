...
const libName = require(path.join(srcFolder, './package.json')).name;
...
return Promise.resolve()
  ...
  // Compile to ES5.
  .then(() => ngc({ project: `${tempLibFolder}/tsconfig.lib-es5.json` })
    .then(exitCode => exitCode === 0 ? Promise.resolve() : Promise.reject())
  ...
  // Copy package files
  .then(() => Promise.resolve()
    .then(() => _relativeCopy('LICENSE', srcFolder, distFolder))
    .then(() => _relativeCopy('package.json', srcFolder, distFolder))
    .then(() => _relativeCopy('README.md', srcFolder, distFolder))
